...
try {
    ...
    // Control de Concurrencia Avanzado (Bloqueo de Doble Click Servidor)
    ...
    $duplicateStmt->execute([':usuario_id' => $baristaId, ':total' => $totalAmount]);
    ...
    $salesStmt = $pdo->prepare('INSERT INTO ventas (usuario_id, total, metodo_pago, fecha_creacion) VALUES (:usuario_id, :total, :metodo_pago, NOW())');
    $salesStmt->execute([':usuario_id' => $baristaId, ':total' => $totalAmount, ':metodo_pago'=> $metodoPago]);
    ...
    foreach ($order as $item) {
        ...
        if ($idProducto <= 0 || $cantidad <= 0 || $precioUnit <= 0.0) throw new RuntimeException('Estructura de ítem corrompida.');

        $calculatedSubtotal += ($precioUnit * $cantidad);
        $productStmt->execute([':id' => $idProducto]);
        $productData = $productStmt->fetch();

        if ($productData && isset($recipeMap[$productData['nombre']])) {
            foreach ($recipeMap[$productData['nombre']] as $insumoNombre => $usoPorUnidad) {
                ...
                if ($inventoryItem) {
                    ...
                    if ((float) $inventoryItem['cantidad_actual'] >= $cantidadRequerida) {
                        $inventoryUpdateStmt->execute([':qty' => $cantidadRequerida, ':id_insumo' => $inventoryItem['id_insumo']]);
                    } else {
    ...
    if (abs($calculatedSubtotal - $totalAmount) > 0.05) throw new RuntimeException('Diferencia de importes. Posible manipulación de JS.');
